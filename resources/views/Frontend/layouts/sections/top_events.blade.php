...
<section class="New-card-event-results-section">
    <div class="New-card-event-results-container">
        ...
        <div class="header">
            ...
            <p class="subtitle">
                Discover the most exciting and trending events happening right now.
            </p>
        </div>
        <!-- Cards Container with Navigation -->
        <div class="filteration-event-results-container-top">
            <div class="New-card-event-cards-container">
                <div class="New-card-event-cards-grid slider-view-top" id="CardsContainer">
                    <!-- Event Card 1 -->
                                    ...
                                    <div class="New-card-event-card">
                                        <a href="{{ url('event/' . $event->uuid) }}">
                                            <img src="{{ asset('storage/' . $event->media->first()->path) }}"
                                                alt="Tech Innovation Summit" class="New-card-event-card-image" />
                                        </a>
                                        @if(auth()->check())
                                            @php
                                            ...
                                            <div class="New-card-event-card-details">
                                                @if($event->format == 0)
                                                    <div class="New-card-event-card-detail">
                                                        <span class="New-card-event-card-detail-icon"><i
                                                                class="fa-thin fa-map-marker-alt"></i></span>
                                                        <span class="New-card-event-card-detail-value">{{ $event->city?->name }}</span>
                                                    </div>
                                                @endif
                                                <div class="New-card-event-card-detail">
                                                    <span class="New-card-event-card-detail-icon"><i
                                                            class="fa-thin fa-calendar-alt"></i></span>
                        ...
                        : (\Carbon\Carbon::parse(
                            $event->start_date . ' ' . \Carbon\Carbon::parse($event->start_time)->format('H:i:s')
                                        ...
                                        <div class="New-card-event-card-footer">
                                            ...
                                            <a href="{{ url('event/' . $event->uuid) }}" class="New-card-event-details-btn">View
                                                Details</a>
                                        </div>
                                    </div>
                    @endforeach
                ...
                <div class="button-card-event">
                    <a href="{{ route('top_events') }}" class="New-card-event-details-btn">
