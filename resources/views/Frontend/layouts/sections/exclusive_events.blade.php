<section class="events-container-exclusive-section js-exclusive-slider">
    <div class="exclusive">
        <div class="header">
                        ...
                        <div class="event-meta-exclusive">
                            <div class="event-actions-exclusive">
                                <span class="event-price-exclusive">
                                    ...
                                    {{ $exclusive_event->tickets->isNotEmpty() && $exclusive_event->tickets->first()->price == 0
                                        ? 'Free'
                                        : ($exclusive_event->tickets->isNotEmpty()
                                            ? number_format($exclusive_event->tickets->first()->price, 0)
                                            : number_format($exclusive_event->tickets->first()?->price, 0)) }}
                                    {{ $exclusive_event->tickets->isNotEmpty()
                                        ? ($exclusive_event->tickets->first()->price == 0 ? '' : $exclusive_event->currency->code)
                                        : '' }}
                                </span>
                                <a href="{{ url('event/' . $exclusive_event->uuid) }}"
