    ...
    <tr>
        ...
        <td>
            ...
            @if ($poster)
                <a href="{{ asset('storage/' . $poster->path) }}" target="_blank">
                    <img src="{{ asset('storage/' . $poster->path) }}" alt="{{ $event->name }}" class="event-poster">
                </a>
            @else
        ...
        <td>
            <div class="d-flex justify-content-center gap-1">
                ...
                <a href="{{ route('admin.events.edit', $event->id) }}" class="btn btn-xs btn-outline-info" title="Edit">
                    <i class="ti ti-pencil ti-sm"></i>
                </a>
                <form action="{{ route('admin.events.destroy', $event->id) }}" method="POST" class="delete-form d-inline">
                    @csrf
                    @method('DELETE')
        ...
        <td>
            <div class="d-flex align-items-center gap-2">
                ...
                <button class="btn btn-xs btn-outline-primary add-gallery-btn" data-event-id="{{ $event->id }}"
                    data-bs-toggle="modal" data-bs-target="#galleryModal-{{ $event->id }}" title="Manage Gallery">
