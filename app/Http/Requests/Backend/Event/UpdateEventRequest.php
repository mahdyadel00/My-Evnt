...
class UpdateEventRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'sub_category_id'               => ['nullable', 'integer', 'nullable'],
            ...
            'end_date.*' => [
                ...
                function ($attribute, $value, $fail) {
                    ...
                    if ($startDate && $value < $startDate) {
                        $fail("The end date at position {$index} must be on or after the start date.");
                    }
                },
            ],
            'start_time' => ['sometimes', 'array'],
            'start_time.*' => ['sometimes', 'nullable', 'regex:/^([0-1]?[0-9]|2[0-3]):[0-5][0-9](:[0-5][0-9])?$/'],
            'end_time' => ['sometimes', 'array'],
            'end_time.*' => [
                'sometimes',
                'nullable',
                ...
                function ($attribute, $value, $fail) {
                    ...
                    if ($startDate && $endDate && $normalizedStartTime && $normalizedEndTime && $startDate === $endDate && $normalizedEndTime <= $normalizedStartTime) {
                        $fail("The end time at position {$index} must be after the start time when the dates are the same.");
                    }
                },
            ],
    ...
    public function messages(): array
    {
        return [
            ...
            'cancellation_policy.string'                        => 'The cancellation policy must be a string.',
